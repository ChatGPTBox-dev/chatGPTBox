...
export function ApiModes({ config, updateConfig }) {
  ...
  const effectiveProviders = useMemo(
    () =>
      applyPendingProviderChanges(customProviders, pendingEditedProvidersById, pendingNewProvider),
    [customProviders, pendingEditedProvidersById, pendingNewProvider],
  )
  const selectedCustomProvider = effectiveProviders.find(
    (provider) => provider.id === providerSelector,
  )
  const hasPendingProviderChanges =
    Boolean(pendingNewProvider) || Object.keys(pendingEditedProvidersById).length > 0
  ...
  const persistApiMode = (nextApiMode) => {
    ...
    if (shouldPersistPendingProviderChanges(nextApiMode, hasPendingProviderChanges)) {
  ...
  const onSaveProviderEditing = (event) => {
    ...
    if (providerEditingId) {
      if (pendingNewProvider && pendingNewProvider.id === providerEditingId) {
        ...
        const existingProvider = selectedCustomProvider || {}
        setPendingEditedProvidersById((currentProviders) => ({
          ...currentProviders,
          [providerEditingId]: buildEditedProvider(
            existingProvider,
            providerEditingId,
            providerName,
            parsedEndpoint,
  ...
  const onSaveEditing = (event) => {
    ...
    if (shouldEditProvider) {
      const selectedProviderId = resolveSelectableProviderId(
        providerSelector,
        effectiveProviders,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', '--spacing': '4px' }}>
      ...
      {shouldEditProvider && (
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', whiteSpace: 'nowrap' }}>
          ...
          <button
            onClick={openEditProviderEditor}
            disabled={!selectedCustomProvider}
            aria-label={t('Edit')}
          >
            <PencilIcon />
          </button>
