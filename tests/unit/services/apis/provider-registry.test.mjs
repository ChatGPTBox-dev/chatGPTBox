...
test('resolveOpenAICompatibleRequest keeps original provider secret when missing provider session has no key', () => {
  const config = {
    customOpenAIProviders: [],
    customApiModes: [],
    providerSecrets: {
  ...
  assert.equal(resolved.providerId, 'legacy-custom-default')
  assert.equal(resolved.requestUrl, 'https://proxy.example.com/v1/chat/completions')
  assert.equal(resolved.apiKey, 'missing-provider-key')
})

test('resolveOpenAICompatibleRequest keeps explicit empty stale provider secret over legacy fallback secret', () => {
  const config = {
    customOpenAIProviders: [],
    customApiModes: [],
    providerSecrets: {
...
test('resolveOpenAICompatibleRequest uses recovered secret slot for builtin-like stale provider id', () => {
  const config = {
    customModelApiUrl: 'https://fallback.example.com/v1/chat/completions',
    providerSecrets: {
  ...
  assert.equal(resolved.secretProviderId, 'openai')
  assert.equal(resolved.requestUrl, 'https://proxy.example.com/v1/chat/completions/')
  assert.equal(resolved.apiKey, 'recovered-shared-key')
})

test('resolveOpenAICompatibleRequest prefers recovered secret slot over stale session key for builtin-like stale provider id', () => {
  const config = {
    customModelApiUrl: 'https://fallback.example.com/v1/chat/completions',
    providerSecrets: {
      openai: 'recovered-shared-key',
...
test('resolveOpenAICompatibleRequest keeps explicit empty recovered secret entry over stale session key', () => {
  const config = {
    customModelApiUrl: 'https://fallback.example.com/v1/chat/completions',
    providerSecrets: {
      openai: '',
...
test('resolveOpenAICompatibleRequest keeps empty builtin-like secret entry over legacy fallback during customUrl recovery', () => {
  const config = {
    customModelApiUrl: 'https://fallback.example.com/v1/chat/completions',
    providerSecrets: {
      openai: '',
