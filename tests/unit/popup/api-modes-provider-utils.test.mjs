...
test('validateProviderEndpointDraft accepts non-standard ollama-style endpoint', () => {
  const draft = validateProviderEndpointDraft('https://localhost:11434/api/chat')

  assert.equal(draft.valid, true)
  assert.equal(draft.parsedEndpoint.chatCompletionsUrl, 'https://localhost:11434/api/chat')
})
