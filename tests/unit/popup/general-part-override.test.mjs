...
test('resolveOverrideCommitContext waits for committed config writes to settle', async () => {
  ...
  let resolveWritesSettled
  const writesSettledPromise = new Promise((resolve) => {
    resolveWritesSettled = resolve
  })
  let persistedConfig = staleConfig
  const contextPromise = resolveOverrideCommitContext(
    () => writesSettledPromise,
    () => persistedConfig,
    'myproxy',
  )
  persistedConfig = latestConfig
  resolveWritesSettled()
